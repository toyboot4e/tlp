...
I believed toylisp would do this conversion: source string → AST → bytecode. It was soon to turn out there were better approaches.
...
I learned from `rust-analyzer`, especially {rowan} and reached another approach.

==== CST

I want my ideal lang to be statically typed, and the parser should be IDE-oriented. toylisp is not that language, still it would be a great if I can learn from {ra}.
...
+
I didn't know the notion of CST. It's basically a tree representation of text (syntax), where each element footnote:[element = sub tree (node) or leaf (token)] is tagged with _syntax kind_ s. One good thing about CST is that each element is _homogeneous_ and _untyped_, so we can easily create CST even for invalid source text in toylisp grammer. I'll know other points of CST in later days.
+
Today I parsed simple arithmetics into CST with {rowan}. I can't run the code yet.

* Jun 25, 2021
+
I added a WIP AST. In context of {rowan}, an AST node is just a wrapper around a CST node; it's just a subtree of CST with accessor methods.
+
I'll make an LSP server while developing `tlp`.
...
* Jul 17, 2021
+
According to the dev guide, rust-analyzer does the following passes: AST → marco-expanded AST (?) → name-resolved IR → type-resolved IR → HIR. While the conversions, RA uses some important patterns: interning, source map pattern and queries. They are (or will be) built around {salsa}.
+
I used to think making my own tool was straight-forward, but {rowan} already changed my mind; there are lightweight wizard's magic that works as a super nice framework. So I hope {salsa} is so, too.
+
