...
+
In the context of `rust-analyzer`, `salsa-2022` *much* simplifies the `hir_def` crate. *We don't have to keep holding `ItemTree`*. Each `Item` is indexed by their `name` as `#[id]`. And that's it. The item IDs keep pointing to the same item. It's even stable on items reordered.
+
...
----
+
It's ridiculously easy! I'm not sure about the details, but it looks like promising. I'm liking `salsa-2022`.
